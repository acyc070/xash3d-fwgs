...
wavdata_t *S_LoadSound( sfx_t *sfx )
{
	...
	else if( sc->rate > SOUND_44k && sc->rate < SOUND_48k ) // some bad sounds
		Sound_Process( &sc, SOUND_48k, sc->width, sc->channels, SOUND_RESAMPLE );
	else if( sc->rate > SOUND_48k && sc->rate < SOUND_88k ) // some bad sounds
		Sound_Process( &sc, SOUND_88k, sc->width, sc->channels, SOUND_RESAMPLE );
	else if( sc->rate > SOUND_88k && sc->rate < SOUND_96k ) // some bad sounds
		Sound_Process( &sc, SOUND_96k, sc->width, sc->channels, SOUND_RESAMPLE );
	else if( sc->rate > SOUND_96k && sc->rate != SOUND_192k ) // some bad sounds
		Sound_Process( &sc, SOUND_192k, sc->width, sc->channels, SOUND_RESAMPLE );
